...
export class MCPClient {
  ...
  async callTool(toolName: string, args: Record<string, unknown>): Promise<MCPResponse> {
    return new Promise((resolve, reject) => {
      const child: ChildProcess = spawn('node', [this.serverPath], {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (response.id === this.requestId && response.result) {
              ...
              if (content) {
                try { resolve(JSON.parse(content)); } catch { resolve({ success: true, output: content }); }
              } else {
      ...
      setTimeout(() => {
        if (!responseReceived) { child.kill(); reject(new Error('Timeout waiting for MCP server response')); }
      }, 120000);
    });
  }
}
